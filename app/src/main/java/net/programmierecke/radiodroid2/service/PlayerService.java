...
public class PlayerService extends JobIntentService implements RadioPlayer.PlayerListener {
    ...
    public void next() {
        ...
        DataRadioStation station = getNextStation(currentStation);
    ...
    public void previous() {
        ...
        DataRadioStation station = getPreviousStation(currentStation);
        if (station != null) {
            if (radioPlayer.isPlaying()) {
                playWithoutWarnings(station);
            } else {
    ...
    private DataRadioStation getNextStation(DataRadioStation currentStation) {
        RadioDroidApp radioDroidApp = (RadioDroidApp) getApplication();
        FavouriteManager favouriteManager = radioDroidApp.getFavouriteManager();
        HistoryManager historyManager = radioDroidApp.getHistoryManager();
        ...
        if (favouriteManager.has(currentStation.StationUuid)) {
            return favouriteManager.getNextById(currentStation.StationUuid);
        }
        
        // 然后在历史记录中查找
        if (historyManager.has(currentStation.StationUuid)) {
            return historyManager.getNextById(currentStation.StationUuid);
        }
        
        // 如果都不在，使用当前的 queue（如果有的话）
        if (currentStation.queue != null) {
            return currentStation.queue.getNextById(currentStation.StationUuid);
        }
        
        return null;
    }

    private DataRadioStation getPreviousStation(DataRadioStation currentStation) {
        RadioDroidApp radioDroidApp = (RadioDroidApp) getApplication();
        FavouriteManager favouriteManager = radioDroidApp.getFavouriteManager();
        HistoryManager historyManager = radioDroidApp.getHistoryManager();
        ...
        // 优先在收藏夹中查找
        if (favouriteManager.has(currentStation.StationUuid)) {
            return favouriteManager.getPreviousById(currentStation.StationUuid);
        }
        
        // 然后在历史记录中查找
        if (historyManager.has(currentStation.StationUuid)) {
            return historyManager.getPreviousById(currentStation.StationUuid);
        }
        
        // 如果都不在，使用当前的 queue（如果有的话）
        if (currentStation.queue != null) {
            return currentStation.queue.getPreviousById(currentStation.StationUuid);
        }
